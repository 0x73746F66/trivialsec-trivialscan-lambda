...
class Webhooks(BaseModel):
    ...
    self_hosted_uploads: Optional[bool] = Field(default=False)
    early_warning_email: Optional[bool] = Field(default=True)
    early_warning_domain: Optional[bool] = Field(default=True)
    early_warning_ip: Optional[bool] = Field(default=True)
    new_findings_certificates: Optional[bool] = Field(default=True)
    new_findings_domains: Optional[bool] = Field(default=True)
...
class MemberAccount(AccountRegistration, DAL):
    ...
    def _billing(self, sub: models.stripe.SubscriptionBase) -> bool:
        self.billing = Billing(
        ...
        if sub.cancel_at_period_end:
            ...
            self.billing.next_due = float(sub.current_period_end) * 1000  # type: ignore
        if (

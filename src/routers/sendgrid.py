...
async def webhook_received(
    ...
    internals.logger.info(
        f"signature {twilio_email_event_webhook_signature} timestamp {twilio_email_event_webhook_timestamp}"
    )
    try:
        if any(
            [
                not twilio_email_event_webhook_signature,
                not twilio_email_event_webhook_timestamp,
            ]
        ) or services.sendgrid.verify_signature(
            raw_body.decode(),
            twilio_email_event_webhook_signature,
            twilio_email_event_webhook_timestamp,
        ):

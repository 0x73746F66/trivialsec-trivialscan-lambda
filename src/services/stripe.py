...
PRODUCTS = {
    ...
    Product.PROFESSIONAL: {
        ...
        "name": "Professional",
        "prices": ["price_1MYHsdGZtHTgMn6l1qnmY2wW", "price_1MYHxpGZtHTgMn6lXUETy2IW"],
    },
...
def get_product(product: Product) -> stripe.Product:
    ...
    try:
        ...
        internals.logger.error(
            f"[get_product] Invalid parameters were supplied to Stripe API: {product_id}"
        )
...
def get_pricing_by_id(price_id: str) -> stripe.Price:
    try:
        ...
        internals.logger.error(
            f"[get_pricing_by_id] Invalid parameters were supplied to Stripe API: {price_id}"
        )
    except AuthenticationError:
        internals.logger.error(
            "[get_pricing_by_id] Authentication with Stripe API failed"
        )
    except StripeError as ex:
        internals.logger.exception(ex)
    except Exception as ex:
...
def create_customer(email: str) -> stripe.Customer:
    try:
        ...
        internals.logger.error(
            f"[create_customer] Invalid parameters were supplied to Stripe API: {email}"
        )
    except AuthenticationError:
        internals.logger.error(
            "[create_customer] Authentication with Stripe API failed"
        )
    except Exception as ex:
        internals.logger.exception(ex)
...
def get_subscription(subscription_id: str) -> stripe.Subscription:
    try:
        ...
        internals.logger.error(
            "[get_subscription] Authentication with Stripe API failed"
        )
    except StripeError as ex:
        internals.logger.exception(ex)
    except Exception as ex:
...
class Webhook:
    ...
    def process(self) -> bool:
        ...
        object_key = (
            f"{internals.APP_ENV}/stripe/{self._created}/{self.event_type}.json"
        )
        internals.logger.warning(
            f"Webhook [{self.event_type}] not processed {self._event_id}"
        )
        return services.aws.store_s3(
            object_key, json.dumps(self._data_object, default=str)
        )
    ...
    def _payment_intent_succeeded(self):
        ...
        for charges_data in self._data_object["charges"]["data"]:
            created = datetime.fromtimestamp(charges_data.get("created")).strftime(
                "%Y%m%d"
            )
            results.append(
                services.aws.store_s3(
